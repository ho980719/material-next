...
import MaterialEditLauncher from "@/components/material/MaterialEditLauncher";
...
export default function MaterialList({ initialItems, zones, page, pageSize, total }: { initialItems: UIMaterial[]; zones: UIZone[]; page: number; pageSize: number; total: number }) {
  ...
  const [selectedZone, setSelectedZone] = useState(searchParams.get("zoneId") ?? "");
  ...
  const search = () => {
    ...
    if (selectedZone) {
      url.searchParams.set("zoneId", selectedZone);
    } else {
